...
**15.11 Period-generic rule.**
All MTD period behaviour in §15.6–§15.10 (document requests and client tasks, requested-document upload completion, awaiting-assignment ownership, overdue-waiting-for-client escalation, client stage wording, figures/review/publish/approval/submission and reopen) is implemented once against a `period_id` and applies **identically to Quarter 1, Quarter 2, Quarter 3, Quarter 4 and the Final Declaration**. There is no quarter-specific branch anywhere in `backend/mtd.py`; the only kind-dependent difference is presentation wording (the Final Declaration is never called a quarter) and its own deadline. A Node.js implementation must keep this single shared code path — no per-quarter handlers, no special-casing of Q1 or the Final Declaration.

**15.12 Verified-behaviour addendum (final pre-export UAT, 2026-06-24).**
The following were proven end-to-end against the running system (backend journey 21/21, one controlled browser pass 35/35 after fixes) and are the frozen behaviour Node.js must reproduce:
- **Upload immutability of scope.** `POST /api/documents/upload` with a `document_id` may never move that document to another case or MTD period: a mismatched `case_id`/`mtd_period_id` returns **400 "This document belongs to a different case or period"**, and the existing period is always preserved. This closes the only cross-period defect found in final UAT.
- **Client action feed.** `GET /api/my-actions` returns MTD document-request tasks with `mtd_period_label` and `link: "/mtd"` (SA tasks keep `link: "/tasks"`), so a period request appears on the client's Action Required screen and opens the correct screen.
- **Net profit.** There is no tax engine: `net_profit` is either typed by the accountant or derived as `income - expenses` when null. The staff figures form recomputes net whenever income or expenses change, so a published version can never contain arithmetically inconsistent figures.
- **Verified permission matrix (403 unless stated).** Client → another client's case/documents/activity: 403. Unassigned accountant → case: 403. Accountant → `record-submission`, `record-prior-submission`, `payment-requests`: 403. Client → `figures`, `record-submission`: 403. Standard ADMIN → `reveal-contact`: 403; SUPER_ADMIN: 200 with mandatory reason (400 if blank) and an audit row. Reassignment immediately revokes the outgoing accountant (403) and grants the incoming one (200).
- **Verified error codes.** 400: blank reason, missing submission reference/date, submission before client approval, duplicate submission, action on a submitted period, cross-period document, service already active. 403: role/permission. 409: client approval against a superseded published version. 422: schema validation.
- **Verified idempotency.** Repeat activation creates no second case, service row or period set. A repeated open document request reuses the same request/task/placeholder. Repeated `_fulfil` (status poll + webhook replay) produces exactly one paid transaction, one receipt and one notification set. Duplicate submission recording is rejected. Opening a page never creates duplicate notifications (one row per recipient per event).
- **Verified audit/history.** Every transition writes an `activity_logs` row; published figure versions accumulate in `published_versions` (a returned-and-resubmitted period keeps the superseded values); document requests are retained after fulfilment; reopen and reassignment reasons are stored.
