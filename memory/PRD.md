...
## 2026-06-24 — MTD UX polish + commercial backend (Parts A-E)
- Part A (presentation only): MtdQuarters shows Case reference / Tax year block, "Your Making Tax Digital service" intro, 5-step What happens next, 6 April 2026 reassurance, "Upload documents" + "No documents requested right now.", Final Declaration wording (never "quarter"), year-summary empty message. Client stage wording via mtd._client_stage(): Getting started / Preparing / Action required / Under review / Ready for your approval / Approved / Submitted / Submitted before joining TaxSimba. Backend states unchanged.
- Part B: packages gain billing_type, vat_treatment, effective_from; PATCH /api/packages/{id} + GET /api/packages/{id}/price-history (SUPER_ADMIN only for changes) with pricing_audit trail. client_services freeze agreed_price, billing_type, billing_frequency, subscription_started_at, payment_session; my-services returns agreed_price + current_master_price.
- Part C: payment requests carry mtd_period_id/label, suggested vs approved amount, VAT breakdown and request_status (SENT/PAID/CANCELLED). Accountant may only recommend (403 on charging); admin/super admin approve and send; payment confirmed server-side once (single receipt/audit/notify).
