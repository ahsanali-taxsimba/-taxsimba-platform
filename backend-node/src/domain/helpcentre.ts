...
export const DEFAULT_FAQS: [string, string, string][] = [
  [
    "Getting Started",
  ...
  [
    "Self Assessment",
    "Why has my accountant requested more information?",
  ...
  [
    "Calculation & Approval",
    "Where can I see my calculation?",
  ...
  [
    "HMRC Submission",
    "When will my return be submitted?",
  ...
  [
    "HMRC Submission",
    "Where can I find my submission confirmation?",
...
/**
 * Seed Help Centre content. The default answers are the canonical wording, so they are kept in
 * step on start-up; admin-authored FAQs are untouched.
 */
export async function seedFaqs(): Promise<void> {
  let order = 0;
  for (const [category, question, answer] of DEFAULT_FAQS) {
    await col("faqs").updateOne(
      { question },
      {
        $set: { category, answer, order, is_active: true },
        $setOnInsert: { id: randomUUID(), question, created_at: nowIso() },
      },
      { upsert: true },
    );
    order += 1;
